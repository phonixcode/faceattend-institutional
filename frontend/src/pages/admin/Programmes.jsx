...
const PAGE_SIZE = 10
const EMPTY_FORM = { name: '', department_id: '', director_id: '' }
...
export default function AdminProgrammes() {
  ...
  const [editing, setEditing] = useState(null)    // programme being edited
  ...
  const openEdit = (prog) => {
    setEditing(prog)
    setForm({ name: prog.name, department_id: prog.department_id || '', director_id: prog.director_id || '' })
    setModal('edit')
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modal === 'edit') {
        await adminApi.updateProgramme(editing.id, form)
  ...
  return (
    ...
    >
      <Card padding={false}>
        {loading ? (
          ...
          <Empty icon={<GraduationCap size={20} />} title="No programmes yet"
            action={<Button onClick={openCreate}><Plus size={14}/>Add Programme</Button>}
          />
        ) : (
          <>
      ...
      <Modal
        open={!!modal}
        onClose={closeModal}
        title={modal === 'edit' ? 'Edit Programme' : 'Add Programme'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Select
            label="Programme Director (optional)"
            value={form.director_id}
            onChange={e => setForm({...form, director_id: e.target.value})}
          >
